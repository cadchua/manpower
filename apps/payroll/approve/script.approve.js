$("document").ready(function(e){
   ...
   $("input[id^=btn-]").click(function(e){
   	      console.log(e);
   	      if(e.target.id=="btn-approve-all"){
   	  	     if(confirm("Approve All Payroll?")){
   	  	    	        $("#progressbar").show();
						param={cmd:"payrollapprove",payroll:w2ui['summarypaygrid'].records,selected:0};
		  ...
		  if(e.target.id=="btn-approve-selected"){
		  	if(confirm("Approve Selected Branch?")){
		  		 $("#progressbar").show();
		  		        var id=w2ui['summarypaygrid'].getSelection();
						param={cmd:"payrollapprove",payroll:w2ui['summarypaygrid'].get(id),selected:1};
						processAjax(payrolURL,param);
						$("#progressbar").css("display","");
			 }
		  }
